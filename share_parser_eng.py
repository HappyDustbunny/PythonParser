...
def main():
    ...
    chunk = []
    all_text = []
    my_file = os.path.join('.', 'cookbook.html')

    with open(my_file, encoding='utf8') as in_file, \
            open("cookbook_output_file.html", "a", encoding='utf8') as out_file:
        read_file = in_file.readlines()

        for line in read_file:
            # if line[0] == '<p class="antal">(Serves':
...
def wrap_serves(line, all_text):
    ...
    all_text.append('                  <button class="share">Share</button>\n')
    all_text.append('              </div>\n')
    return all_text
